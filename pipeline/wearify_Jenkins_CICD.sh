...
if [ "$operationType" -eq 1 ]; then
    ...
    aws s3 cp ./jenkins_home.tgz s3://wearify-infra-state-bucket-new/jenkins_home.tgz
    ...
    aws s3 cp s3://wearify-infra-state-bucket-new/jenkins_home.tgz ./jenkins_home.tgz
